...
import { memberFormSchema } from "@/lib/Schema/memberSchema"; // Imported the schema from above
  ...
  useEffect(() => {
    if (isOpen) {
      setFormData({
        ...
        image: initialData?.image || undefined, // Keep existing image string references
      });
      setFileMeta({
      ...
      <div className="relative bg-white w-full max-w-3xl rounded-2xl border border-slate-100 shadow-xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200 z-10 text-xs">
        ...
        <div className="p-6 overflow-y-auto space-y-5 flex-1">
          ...
          <div className="space-y-1.5">
            ...
            <div className={`w-full border rounded-xl bg-white overflow-hidden transition-all flex flex-col ${errors.description ? 'border-rose-400 focus-within:border-rose-500 focus-within:ring-rose-500/30' : 'border-slate-200 focus-within:border-indigo-500 focus-within:ring-indigo-500/30'} focus-within:ring-1`}>
              {/* Toolbar */}
              ...
              {editorTab === "write" ? (
                <textarea id="member-modal-textarea" name="description" value={formData.description} onChange={handleInputChange} rows={6} placeholder="Write member bio, achievements, or role description..." className="w-full p-4 focus:outline-none resize-y min-h-37.5 border-0 text-slate-700 leading-relaxed font-mono text-[11px] bg-transparent" />
              ) : (
                <div className="w-full p-5 min-h-37.5 max-h-70 overflow-y-auto bg-slate-50/40 text-slate-700 leading-relaxed prose prose-slate max-w-none prose-sm" dangerouslySetInnerHTML={{ __html: formData.description.replace(/\n/g, "<br/>") }} />
              )}
            </div>
