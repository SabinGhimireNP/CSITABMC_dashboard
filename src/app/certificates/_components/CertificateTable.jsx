...
import { ArrowUpDown, MoreHorizontal, Pencil, Trash2, AlertTriangle, X, Copy } from "lucide-react";
...
export function CertificateTable({
  ...
  const handleMenuToggle = useCallback((e, certId) => {
    e.stopPropagation();
    if (activeMenuId === certId) { setActiveMenuId(null); return; }
  ...
  const handleConfirmDelete = () => {
    if (certToDelete) { onDeleteCertificate(certToDelete.id); setCertToDelete(null); }
  };
  ...
  const DropdownMenu = ({ cert }) =>
    createPortal(
      ...
      >
        <button type="button"
          onClick={() => { if (onEditCertificate) onEditCertificate(cert); setActiveMenuId(null); }}
        ...
        <button type="button"
          onClick={() => { if (onDuplicateCertificate) onDuplicateCertificate(cert); setActiveMenuId(null); }}
        ...
        <button type="button"
          onClick={() => { setCertToDelete(cert); setActiveMenuId(null); }}
    ...
    <div className="w-full flex flex-col">
        ...
        <table className="w-full text-left border-collapse min-w-150 table-fixed">
          <thead>
            <tr className="border-b border-slate-200/80 bg-slate-50/70 text-slate-500 font-semibold text-xs select-none">
              ...
              <th onClick={() => onHeaderClick("fullName")} className="py-3 px-4 cursor-pointer hover:bg-slate-100/50 transition-colors w-[25%]">
                <div className="flex items-center gap-1.5">Full Name <SortIndicator field="fullName" /></div>
              </th>
              <th onClick={() => onHeaderClick("event")} className="py-3 px-4 cursor-pointer hover:bg-slate-100/50 transition-colors w-[40%]">
                <div className="flex items-center gap-1.5">Event <SortIndicator field="event" /></div>
              </th>
              <th className="py-3 px-4 w-[15%]">Project Status</th>
              <th onClick={() => onHeaderClick("createdAt")} className="py-3 px-4 cursor-pointer hover:bg-slate-100/50 transition-colors w-[14%]">
          ...
          <tbody className="divide-y divide-slate-100 text-slate-700 text-xs">
            {certificates.length === 0 ? (
              ...
              certificates.map((cert) => (
                <tr key={cert.id} className="hover:bg-slate-50/40 transition-colors group h-14 bg-white">
                  <td className="py-2 px-4 font-semibold text-slate-900 tabular-nums truncate">{cert.id}</td>
      ...
      {certToDelete && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-150">
          ...
          >
            <button type="button" onClick={() => setCertToDelete(null)}
            ...
            <div className="flex gap-3.5">
              ...
              <div className="space-y-1.5 flex-1">
                <h3 className="text-sm font-bold text-slate-900 tracking-tight">Delete certificate?</h3>
                <p className="text-xs text-slate-500 leading-relaxed font-normal">
                  Are you sure you want to delete the certificate for{" "}
                  <span className="font-semibold text-slate-800">"{certToDelete.fullName}"</span>? This cannot be undone.
                </p>
              </div>
            </div>
