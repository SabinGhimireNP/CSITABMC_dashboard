...
export function AdminTabs({ activeTab, onTabChange, searchQuery, onSearchChange }) {
  const tabs = [
        ...
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              {tab.id === "inactive" && (
                <span className={`h-1.5 w-1.5 rounded-full ${isActive ? "bg-indigo-600 animate-pulse" : "bg-slate-400"}`} />
              )}
            </button>
          );
        })}
